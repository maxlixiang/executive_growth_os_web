...
export async function buildGrowthContext(
  ...
  const [state, focuses, progress, daily, evidence, gaps, monthly, quarterly] = await Promise.all([
    ...
    client.from("practice_evidence").select("evidence_level, context, user_role, action, decision, outcome, limitations, next_evidence_needed, capability_id").eq("user_id", userId).filter("journey_id", "eq", journeyId).gte("created_at", since).order("created_at", { ascending: false }).limit(20),
