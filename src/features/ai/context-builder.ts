...
export async function buildGrowthContext(
  ...
  const [state, focuses, progress, daily, evidence, gaps, monthly, quarterly] = await Promise.all([
    ...
    client.from("daily_reflections").select("reflection_date, analysis, responsibility_hint").eq("user_id", userId).gte("created_at", since).order("created_at", { ascending: false }).limit(20),
    client.from("practice_evidence").select("evidence_level, context, user_role, action, decision, outcome, limitations, next_evidence_needed, capability_id").eq("user_id", userId).gte("created_at", since).order("created_at", { ascending: false }).limit(20),
    client.from("growth_gaps").select("gap_type, title, detail, status, capability_id, concept_id").eq("user_id", userId).eq("status", "open").order("created_at", { ascending: false }).limit(30),
    client.from("monthly_reviews").select("period_start, review_markdown").eq("user_id", userId).order("period_start", { ascending: false }).limit(1).maybeSingle(),
    client.from("quarterly_reviews").select("period_start, assessment_markdown, executive_level_gaps, next_quarter_focus").eq("user_id", userId).eq("status", "completed").order("period_start", { ascending: false }).limit(1).maybeSingle(),
  ]);
